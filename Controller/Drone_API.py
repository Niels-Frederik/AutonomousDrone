		...
		for ip in in_pipe:	
			if ip == parent_pipe:
				...
				if cmd == "die!":
					...
					write2pipe = open(VidPipePath,"wb+", 0)
					suicide = False
					inited = True
					preinited = False
					unsureMode = True
...
###
    ...
    exit = False
    while drone.getBattery()[0] == -1:	time.sleep(0.1) and not exit	# Waits until the drone has done its reset
    ...
    while not stop:
            ...
            if key == " ":
                ...
                #cv2.imshow(".", drone.VideoImage)
            elif key == "v":    
                drone.slowVideo(True) #drone.startVideo(True)
                drone.mp4Video(True)
                drone.frontCam(True)
                print('done')
            elif key == "h":    drone.showVideo(True)
            #elif key == "k":    drone.relMove(0, 1, 0, 0, 0, 0)     #This kills the drone completely
            elif key !="":     stop = True
